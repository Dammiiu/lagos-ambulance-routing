            ...
            with st.spinner("Running Time-Aware Dijkstra routing..."):
                ...
                if current_veh_node is not None and prev_result:
                    ...
                    if is_on_leg2:
                        ...
                        if h_node:
                            h_path, h_time = shortest_path(G, current_veh_node, h_node, blocked_edges=st.session_state["blocked_edges"])
                            if h_time < float("inf"):
                        ...
                        veh_path, veh_time = shortest_path(G, current_veh_node, inc_node, blocked_edges=st.session_state["blocked_edges"])
                        if veh_time < float("inf"):
                            ...
                            if prev_result.get("has_leg2") and prev_result.get("leg2_hospital_node"):
                                ...
                                h_path, h_time = shortest_path(G, inc_node, h_node, blocked_edges=st.session_state["blocked_edges"])
                                if h_time < float("inf"):
                                    result["leg2_path"] = h_path
                                    result["leg2_time_min"] = h_time
                    ...
                    result = two_leg_route(G, nodes_gdf, inc_node, inc_type, stations_gdf, blocked_edges=st.session_state["blocked_edges"])
            ...
            if result is None:
                ...
                if current_veh_node is None:
                    if veh_node is not None and veh_mode not in ("At dispatch station",):
                        veh_path, veh_time = shortest_path(G, veh_node, inc_node, blocked_edges=st.session_state["blocked_edges"])
                        if veh_time < float("inf"):
                            result["leg1_path"]      = veh_path
                            result["leg1_time_min"]  = veh_time
    ...
    with panel_col:
        ...
        if result:
            ...
            _result_html = (
                ...
                f'<div style="margin-bottom:0.5rem">{_badge_type}{_badge_custom}{_badge_leg}</div>'
    ...
    with map_col:
        ...
        if st.session_state.get("result"):
            ...
            with st.container():
                ...
                st.session_state["sim_progress"] = prog_val
